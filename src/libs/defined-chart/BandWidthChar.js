...
export const BandWidthChart = React.createClass({
	displayName: "BandWidthChart",
	...
	_tooltipHtml: function _tooltipHtml(e,d, position) {
		...
		var xData =  values(data).map(function(stack,index){
			return x(stack);
		});
		var xValue = xData[xIndex - 1];
		var yData = values(data).map(function(stack,index){
			return y(stack);
		});
		var yValue = yData[xIndex - 1];

        var arr = values(cityEquipmentData[0]);
        var yValue1 = y(arr[xIndex - 1]);
		...
		return [this.props.tooltipHtml([ yValue, yValue1 ], xValue), xPos, yPos];
	},

...
class HalfHourChar extends Component{
    	...
    	return {
    		...
    		tooltipHtml(label,value){
    			var data = _this.charData,
                    dateStr = new Date(value["x"]).Format("yyyy-MM-dd hh:mm"),
                    htl = [],
    			    tempData = [];
	...
	render(){
		...
		const { levelWarningHalfHourProps } = this.props;

		if(isEmptyObj(levelWarningHalfHourProps) || isEmptyObj(levelWarningHalfHourProps["param"])){
			return false;
		}

		this.charData = this.adapterCharData(levelWarningHalfHourProps["param"]);
...
exports.HalfHourChar = HalfHourChar;
